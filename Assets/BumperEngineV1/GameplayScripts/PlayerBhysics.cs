...
public class PlayerBhysics : MonoBehaviour {
    ...
    Vector3 KeepNormal;
    public float KeepNormalCounter { get; set; }
    ...
    void GeneralPhysics()
    {
        ...
        if (UseSphereToGetNormal)
        {
            ...
            Grounded = Physics.Raycast(transform.position, -CollisionPointsNormal, out hit, RayToGroundDistance);
        }
        else
        {
            Grounded = Physics.Raycast(transform.position, -transform.up, out hit, RayToGroundDistance);
        }
...
*/
    ...
    {
		float StickingSpeedFactor = (rigidbody.velocity.magnitude * 0.06f);
