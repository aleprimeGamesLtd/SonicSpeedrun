...
public class Action01_Jump : MonoBehaviour {
    ...
    void Update()
	{
		...
		if (Actions.Action02 != null) {
			...
			if (Counter > 0.08f && Input.GetButtonDown ("X") && Actions.Action02Control.HasTarget && Actions.Action02.HomingAvailable) {
			...
			if (Counter > 0.08f && Input.GetButtonDown ("A") && Actions.Action02.HomingAvailable && Actions.Action08 == null) {
				if (Actions.Action02Control.HomingAvailable) {
					sounds.AirDashSound ();
		...
		//Do a Bounce Attack
		if (Input.GetButtonDown("B") && Actions.Action06.BounceAvailable)
		{
