...
public class Action03_SpinDash : MonoBehaviour {
	...
	void FixedUpdate ()
    {
        Actions.Action01.JumpBall.SetActive(true);
    ...
    void Release()
    {
        ...
        else
        {
			...
			Player.rigidbody.velocity = charge * (PlayerSkinTransform.forward);
    ...
    void Update()
    {
        ...
        CharacterAnimator.SetInteger("Action", 3);
        ...
        if (Player.RawInput.sqrMagnitude < 0.9f)
        {
            ...
            {
                CharRot = Quaternion.LookRotation(RawPrevInput * Vector3.Dot(Player.MainCamera.transform.forward, Player.GroundNormal), Vector3.up);
            }
            else
            {
        ...
        else
        {
            if (GetComponent<Objects_Interaction>().Mode2D)
            {
                CharRot = Quaternion.LookRotation(Vector3.Scale(Player.rigidbody.velocity, transform.right), Vector3.up);
            }
            else
            {
                CharRot = Quaternion.LookRotation(Player.rigidbody.velocity, Vector3.up);
            }
